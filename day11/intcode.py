...
def run(filename, threadname, inputbuf, outputbuf):
    with open(filename) as f:
        ...
        while program[idx] != 99:
            ...
            if opcode == 1 or opcode == 2:
                value0 = read_value(program, idx, get_mode(param_modes, 0), relative_base)
                value1 = read_value(program, idx + 1, get_mode(param_modes, 1), relative_base)
                if opcode == 1:
                    # add
                    write_value(value0 + value1, program, idx + 2, get_mode(param_modes, 2), relative_base)
                else:
                    # opcode == 2
                    # multiply
                    write_value(value0 * value1, program, idx + 2, get_mode(param_modes, 2), relative_base)
                ...
                # input
                write_value(inputbuf.get(), program, idx, get_mode(param_modes, 0), relative_base)
                idx += 1
                ...
                if value0 < value1:
                    ...
                    write_value(1, program, idx + 2, get_mode(param_modes, 2), relative_base)
                else:
                    # Otherwise, it stores 0
                    write_value(0, program, idx + 2, get_mode(param_modes, 2), relative_base)
                idx += 3
            elif opcode == 8:
                # equals
                value0 = read_value(program, idx, get_mode(param_modes, 0), relative_base)
...
#filename = sys.argv[1]
#run(filename, "main", UserInput(), UserOutput())
